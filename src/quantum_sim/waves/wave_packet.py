...
class WavePacket(WaveFunction):
    ...
    def __init__(self, plane_waves: list[PlaneWave], time: float = 0.0):
        ...
        self._amplitudes = np.array(
            [pw.amplitude for pw in plane_waves], dtype=complex
        ).ravel()

        self._k_vectors = np.array(
            [pw.wave_number for pw in plane_waves], dtype=float
        ).ravel()

        self._omegas = np.array(
            [pw.angular_frequency for pw in plane_waves], dtype=float
        ).ravel()
