...
# Potentiel pour un puits infini
well = InfiniteWell(a=-15.0, b=15.0, V_wall=5e-37)
V = well.evaluate(solver.x_grid)
...
# Affichage du potentiel du puits infini
ax.plot(x * 1e9, V / np.max(V) * np.max(prob), color='k', lw=1.5, label='Infinite Well Potential')

# Configuration de l'affichage
ax.set_xlabel('x (nm)')
ax.set_ylabel(r'$|\psi(x,t)|^2$')
ax.set_title("Propagation d'un paquet gaussien dans un puits infini")
ax.legend()
